...
| Examples | Incorrect calculations affecting recommendations. A numerical value or statistic presented with no source. A value whose cited source does not contain it, or that comes from a non-credible source. Contradictory findings and recommendations. Missing assumptions that invalidate conclusions. Unsupported strategic recommendations. | Missing supporting evidence. Weak risk assessment. Poor client tailoring. Missing implementation considerations. | Formatting inconsistencies. Citation formatting issues. Redundant language. Minor wording concerns. Grammatical errors. Rounding differences on figures explicitly qualified as approximate (e.g., "roughly," "approximately"). |
